...
def visualize_pts(pts, fig=None, bgcolor=(1, 1, 1), fgcolor=(0, 0, 0),
                  show_intensity=False, size=(10, 10), draw_origin=True):
    ...
    if show_intensity:
        G = mlab.points3d(pts[:, 0], pts[:, 1], pts[:, 2], pts[:, 3], mode='point',
                          colormap='gnuplot', scale_factor=2, figure=fig)
    else:
...
def draw_sphere_pts(pts, color=(0, 1, 0), fig=None, bgcolor=(0, 0, 0), scale_factor=0.2):
    ...
    if fig is None:
        fig = mlab.figure(figure=None, bgcolor=bgcolor, fgcolor=None, engine=None, size=(10, 10))
...
def draw_scenes(points, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None):
    ...
    fig = draw_multi_grid_range(fig, grid_size=2, bv_range=(-6, -6, 6, 6))
    if gt_boxes is not None:
        corners3d = boxes_to_corners_3d(gt_boxes)
        fig = draw_corners3d(corners3d, fig=fig, line_width=5, color=(1, 0, 0), max_num=100)

    if ref_boxes is not None and len(ref_boxes) > 0:
        ref_corners3d = boxes_to_corners_3d(ref_boxes)
        if ref_labels is None:
            fig = draw_corners3d(ref_corners3d, fig=fig, color=(0, 0, 1), cls=ref_scores, max_num=100)
        else:
            for k in range(ref_labels.min(), ref_labels.max() + 1):
                cur_color = tuple(box_colormap[k % len(box_colormap)])
                mask = (ref_labels == k)
                fig = draw_corners3d(ref_corners3d[mask], fig=fig, color=(0, 0, 1), cls=ref_scores[mask], max_num=100)
    mlab.view(azimuth=-179, elevation=50.0, distance=10.0, roll=90.0)
    return fig
    ...
    for n in range(num):
        ...
        if cls is not None:
            if isinstance(cls, np.ndarray):
                mlab.text3d(b[6, 0], b[6, 1], b[6, 2], '%.2f' % cls[n], scale=(0.15, 0.15, 0.15), color=color, figure=fig)
            else:
                mlab.text3d(b[6, 0], b[6, 1], b[6, 2], '%s' % cls[n], scale=(0.15, 0.15, 0.15), color=color, figure=fig)
